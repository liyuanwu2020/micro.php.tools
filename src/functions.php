...
if (!function_exists('core_autoloader')) {
    function core_autoloader($class) {
        $filename = CORE_LIB_ROOT . $class . '.php';
        if (is_readable($filename)) include $filename;
    }
}

if (!function_exists('xhprofRegister')) {
    function xhprofRegister() {
        global $experiment;
        global $anticipated;
        $experiment->record();
        if (extension_loaded('xhprof') && (bccomp($experiment->end(false), $anticipated, 6) > 0)) {
            $xhprof_data = xhprof_disable();
            //write to file
            LogXhprofRuntime::info('xhprof', $xhprof_data);
            //todo 文件 -> filebeat -> 队列 -> 处理
        }

        error_get_last() and LogRuntime::error('error_get_last', error_get_last());
    }
}
...
if (!function_exists('readableTime')) {
    function readableTime($size, $scale = 3): string
    {
        ...
        return functions . phpround($operand * $size, $scale);
    }
}
...
if (!function_exists('readableBytes')) {
    function readableBytes($size, $scale = 3): string
    {
        ...
        return functions . phpround($size / $operand, $scale);
    }
}
